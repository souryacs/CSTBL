...
##-----------------------------------------------------
""" this function computes the false positive, false negative
and symmetric difference between the inferred supertree and an input tree """
def Compute_FP_FN_RF_SingleTree(Src_Tree, Derived_Tree):
  curr_src_tree_taxa = Src_Tree.infer_taxa().labels()
  pruned_tree = dendropy.Tree(Derived_Tree)
  #pruned_tree_taxa = pruned_tree.infer_taxa().labels()

  # comment - sourya
  pruned_tree.retain_taxa_with_labels(curr_src_tree_taxa)
  #pruned_tree.retain_taxa(curr_src_tree_taxa)
  
  ## add - sourya
  #l = []
  #for t in pruned_tree_taxa:
    #t1 = t
    #t1.replace('_', ' ')
    #if t not in curr_src_tree_taxa and t1 not in curr_src_tree_taxa:
      #l.append(t)
  #pruned_tree.prune_taxa_with_labels(l) 
  ## end add - sourya
  
  tup = Src_Tree.false_positives_and_negatives(pruned_tree)
  FP = tup[0]
  FN = tup[1]
  RF = FP + FN
  normRF = (RF * 1.0) / (2 * len(curr_src_tree_taxa) - 6)
  return FP, FN, RF, normRF
